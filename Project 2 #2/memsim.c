...
//for VMS
VMSFrame_t *vmsTables; //Pointer to array of tables.
queueStart_t vmsCPQueueStart;
queueStart_t vmsDPQueueStart;

//for LRU
struct LRUFrame *LRUTable;
...
void PrintResults(void)
{
    ...
    if(strcmp(mode,"debug") == 0)
        printf("hit rate: %.2f\n", (double)((1000000-numReads)*100/(double)1000000)); //Hit Ratio
}
...
void initializeVMS(void)
{
    ...
    while(i<numFrames)
    {
        pageEntryLink->isDirty = 'R';//intializing all page frames to read type.
        pageEntryLink++;
        i++;
    }
}
...
{
    queue_t *Qelement, *pTempEle, *pTemp1Element;
...
void VMS(unsigned int addr, char rw)
{
    ...
    if(numCleanPages <= (numFrames/2))
    {
        ...
        {   
            ...
            PutAtQHead((queueStart_t *)&vmsCPQueueStart, (queue_t *)pageEntryLink); // Add to clean page list
            numCleanPages++;
            numReads++;
            return;
        }
        ...
        if(entryFound)
        {
            ...
            if((pageEntryLink->isDirty == 'R') && (rw == 'W'))
                pageEntryLink->isDirty = 'W';   //dirty bit gets updated
            return;
        }
        else
        {
            ...
            while(i <= (numFrames/2))
            {
                if(!pageEntryLink->isValid)
                {
                    ...
                    PutAtQHead((queueStart_t *)&vmsCPQueueStart, (queue_t *)pageEntryLink); //Add the clean list
                    numCleanPages++;
                    numReads++;
                    return;
                }
...
//Least recently Uses implementation
void LRU(unsigned int addr, char rw)
{
    ...
    LRU_PageTableEntries = LRUPage();

    if(LRU_PageTableEntries->isDirty == 'W')
        numWrites++;
    numReads++; //new page will be read from disk.
    LRU_PageTableEntries->num2 = num2;//updating the pageFrame entry
    LRU_PageTableEntries->num1 = 0;
    LRU_PageTableEntries->isDirty = 'R';    //intialize it to read type.
    if(strcmp(mode,"debug")==0)
...
int main(int argc, char *argv[])
{
    ...
    {
        LRUTable = (struct LRUFrame *)calloc((size_t)(sizeof(struct LRUFrame)*numFrames), (size_t)1);// array space is allocated
